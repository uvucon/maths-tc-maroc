...
import { courses, getCourse, type Course } from './data'
...
function Shell({children,path,go}:{children:ReactNode,path:string,go:(to:string)=>void}) {
  ...
  const links = [['/','Accueil','⌂'],['/programme','Programme','▦'],['/revision','Révision','↻'],['/profil','Profil','○']]
...
function Curriculum({go}:{go:(to:string)=>void}) {
  ...
  return <><section className="page-intro"><span className="eyebrow">15 chapitres · Tronc Commun Sciences</span><h1>Ton programme,<br/><em>sans zones d’ombre.</em></h1><p>Un parcours fidèle à la progression marocaine, avec des objectifs clairs et des ressources de recherche ciblées.</p></section><div className="filter-bar"><div>{([['all','Tout'],[1,'Semestre 1'],[2,'Semestre 2']] as const).map(([v,l])=><button className={filter===v?'active':''} onClick={()=>setFilter(v)} key={v}>{l}</button>)}</div><span>{progress.completed.length} / 15 terminés</span></div><section className="curriculum-grid">{shown.map(c=><CourseCard course={c} go={go} key={c.id}/>)}</section><SourceNote/></>
}
...
function CoursePlayer({id,go}:{id:string,go:(to:string)=>void}) {
  ...
  const [tab,setTab]=useState<'cours'|'quiz'>('cours'); const [answers,setAnswers]=useState<number[]>([]); const [submitted,setSubmitted]=useState(false)
...
function Profile() {
 ...
 return <><section className="page-intro compact"><span className="eyebrow">Mon espace</span><h1>Des efforts visibles,<br/><em>pas de comparaison.</em></h1></section><div className="profile-grid"><section className="profile-card"><div className="avatar">Y</div><div><h2>{progress.learner}</h2><p>Élève · Tronc Commun Sciences</p></div><div className="level-pill">Niveau {level}<small>{xp} XP</small></div></section><section className="badges-card"><span className="eyebrow">Badges</span><h2>Petites victoires</h2><div className="badges">{badges.map(([name,desc,earned],i)=><div className={earned?'earned':''} key={String(name)}><span>{['✦','↗','◷','½','✓'][i]}</span><b>{name}</b><small>{desc}</small></div>)}</div></section><section className="settings-card"><span className="eyebrow">Données locales</span><h2>Ton progrès reste sur cet appareil</h2><p>Aucun compte, aucun serveur : MathSprint sauvegarde automatiquement ton parcours dans le stockage local du navigateur.</p>{confirm?<div className="confirm"><b>Effacer toute la progression ?</b><button onClick={()=>{reset();setConfirm(false)}}>Oui, tout effacer</button><button onClick={()=>setConfirm(false)}>Annuler</button></div>:<button className="danger-button" onClick={()=>setConfirm(true)}>Réinitialiser ma progression</button>}</section></div><SourceNote/></>
}
...
export default function App(){const {path,go}=useRoute();let page:ReactNode;if(path==='/programme')page=<Curriculum go={go}/>;else if(path==='/revision')page=<Review go={go}/>;else if(path==='/profil')page=<Profile/>;else if(path.startsWith('/cours/'))page=<CoursePlayer id={decodeURIComponent(path.slice(7))} go={go}/>;else page=<Dashboard go={go}/>;return <Shell path={path} go={go}>{page}</Shell>}
