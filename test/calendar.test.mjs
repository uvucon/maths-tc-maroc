...
function authMiddleware(req, res, next) {
  ...
  if (!auth || auth !== 'Cookie valid') return res.status(401).json({ error: 'Unauthorized' })
  req.userId = 'test-user'
  next()
}

test('calendar endpoints', async (t) => {
  // Clear any leftover data from db just in case
  ...
  try {
    ...
    const createRes = await fetch(`${base}/api/calendar/sessions`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: 'Cookie valid' },
      body: JSON.stringify({
    ...
    const listRes = await fetch(`${base}/api/calendar/sessions`, {
      headers: { Authorization: 'Cookie valid' }
    })
